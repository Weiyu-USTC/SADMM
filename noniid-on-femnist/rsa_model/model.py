...
from baseline_constants import ACCURACY_KEY, lr, graph
...
class Model(ABC):
    ...
    def __init__(self, seed, lr, optimizer=None):
        ...
        with graph.as_default():
            ...
            self.sign_client_server = [np.zeros((784, 62)), np.zeros(62)]
            self.features, self.labels, self.sign_client_server_0, self.sign_client_server_1, self.lr_train, self.train_op, self.eval_metric_ops, self.loss = self.create_model()
        ...
        with graph.as_default():
            ...
            self.flops = tf.profiler.profile(
                graph, run_meta=metadata, cmd='scope', options=opts).total_float_ops
    ...
    def set_lr_client(self, round_number):
        self.lr = lr / math.sqrt(round_number)

    def set_sign_client_server(self, model_server):
        model_client = self.get_params()
        self.sign_client_server[0] = np.sign(model_client[0] - model_server[0])
        self.sign_client_server[1] = np.sign(model_client[1] - model_server[1])
            ...
            with graph.as_default():
                self.sess.run(self.train_op,
                              feed_dict={
                                  ...
                                  self.sign_client_server_0: self.sign_client_server[0],
                                  self.sign_client_server_1: self.sign_client_server[1],
