...
class Server:
    ...
    def train_model(self, num_epochs=1, batch_size=10, minibatch=None, clients=None):
        ...
        for c in clients:
            ...
            self.updates.append((num_samples, model_client))
    ...
    def update_model(self):
        ...
        for i in range(len(self.updates)):
            if i in byzantine:
                # _, model_client = self.updates[regular[0]]
                _, model_client = self.updates[i]
                model_client[0] *= -100
                model_client[1] *= -100
            else:
                _, model_client = self.updates[i]
            for j in range(len(base)):
                base[j] += model_client[j] / len(self.updates)

        self.model = base
        self.updates = []

    # Consider the unbalance data (but not fit for the Byzantine case)
    # def update_model(self):
    #     total_weight = 0.
    #     base = [0] * len(self.updates[0][1])
    #     for i in range(len(self.updates)):
    #         if i in byzantine:
    #             client_samples, client_model = self.updates[regular[0]]
    #         else:
    #             client_samples, client_model = self.updates[i]
