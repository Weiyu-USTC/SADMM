...
class Server:
    ...
    def update_model(self, grad_list):
        num_clients = grad_list[0].shape[0]
        for i in range(len(self.model)):
            grad_list_reshape = np.reshape(grad_list[i], (num_clients, -1))
            guess = np.median(grad_list_reshape, axis=0)
            if i == 0 :
                guess = np.reshape(guess, (784, 62))
            elif i == 1:
                guess = np.reshape(guess, (62, ))
